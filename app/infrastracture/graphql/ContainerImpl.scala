...
import core.caches.{MasterCacheImpl, MasterCacheModule}
import domain.{Id, User}
import javax.inject.Inject
import org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder
import pdi.jwt.JwtJson
import play.api.libs.json.JsObject
import adapter.repositories._
import usecase.{AccountService, BudgetService, IncomeSpendingService, UserService}
import core.utils.TryUtil._
...
class ContainerImpl @Inject()()(
  ...
  override def resolveUserByToken(tokenOpt: Option[String]): Future[Option[User]] =
    tokenOpt match {
      case Some(token) =>
        val value = token.replaceAll("Bearer ", "").trim
